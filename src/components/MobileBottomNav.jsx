...
export default function MobileBottomNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-lg border-t border-choco-200/60 shadow-lg px-2 py-1.5 flex items-center justify-around">
      {navItems.map((item) => {
        ...
        return (
          <Link
            ...
            className={`relative flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all duration-200 ${
              isActive ? 'text-choco-900 font-bold scale-105' : 'text-choco-500 hover:text-choco-800'
            }`}
          >
            <div className="relative">
              {item.icon}
              {item.badge > 0 && (
                <span className="absolute -top-1.5 -right-2.5 bg-gold-500 text-white text-[9px] font-bold px-1.5 py-0.2 rounded-full min-w-[16px] text-center shadow-xs">
